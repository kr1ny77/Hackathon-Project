...
echo "=== FIX v16 — Scheduler de-duplication & Intake Menu ==="

python3 << 'PYEOF'
import os

# 1. Fix scheduler.py to prevent duplicate messages and use correct scheduled_time
path = "/opt/medicine-reminder/bot/app/services/scheduler.py"
with open(path) as f:
    content = f.read()

old_scheduler_block = """        for (tid, med_name), infos in groups.items():
...
new_scheduler_block = """        for (tid, med_name), infos in groups.items():
            count = len(infos); primary = infos[0]
            # Use the reminder time as scheduled_time so duplicates are caught by backend
...
if old_scheduler_block in content:
    content = content.replace(old_scheduler_block, new_scheduler_block)
